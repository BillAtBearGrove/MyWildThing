...
#include "../include/Config.h"  // for maxBreak
...
int xRest = a2dMid; // nominal "At rest" position for joystick (A2D)
int yRest = a2dMid; // nominal "At rest" position for joystick (A2D)
...
// Joystick Values - Start at middle position
int joySelect = 0; // <1> Tethered (default) or <2> Occupant <0> Error
int joyPosX = a2dMid; // (A2D)
int joyPosY = a2dMid; // (A2D)
...
int joyFaultBand = (a2dMax * 0.05); // # of A2D units at each end of joystick range that we interpret as a fault (aka. at/near 0 and at/near full scale)
