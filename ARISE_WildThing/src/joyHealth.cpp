...
struct joyInput joyReset(struct joyInput joyInputs_) {
    joyInputs_.O.x.center = a2dMid;
    joyInputs_.O.y.center = a2dMid;
...
struct joyInput joyProcessing(struct joyInput joyInputs_) {
    ...
    joyInputs_.O.x.health = max(0, min(1000, joyInputs_.O.x.health + ( abs(joyInputs_.O.x.raw - joyInputs_.O.x.ref) < joyMaxDelta  ? joyIncGood : joyDecNoise ) ));
    joyInputs_.O.y.health = max(0, min(1000, joyInputs_.O.y.health + ( abs(joyInputs_.O.y.raw - joyInputs_.O.y.ref) < joyMaxDelta  ? joyIncGood : joyDecNoise ) ));
    joyInputs_.T.x.health = max(0, min(1000, joyInputs_.T.x.health + ( abs(joyInputs_.T.x.raw - joyInputs_.T.x.ref) < joyMaxDelta  ? joyIncGood : joyDecNoise ) ));
    joyInputs_.T.y.health = max(0, min(1000, joyInputs_.T.y.health + ( abs(joyInputs_.T.y.raw - joyInputs_.T.y.ref) < joyMaxDelta  ? joyIncGood : joyDecNoise ) ));

    // modify health based on OutOfRange (inc/decr if raw measure is outside diagnostic bands)
    joyInputs_.O.x.health = max(0, min(1000, joyInputs_.O.x.health + ( (joyInputs_.O.x.raw>joyFaultBand && joyInputs_.O.x.raw<a2dMax-joyFaultBand) ? joyIncGood : joyDecOOR ) ));
    joyInputs_.O.y.health = max(0, min(1000, joyInputs_.O.y.health + ( (joyInputs_.O.y.raw>joyFaultBand && joyInputs_.O.x.raw<a2dMax-joyFaultBand) ? joyIncGood : joyDecOOR ) ));
    joyInputs_.T.x.health = max(0, min(1000, joyInputs_.T.x.health + ( (joyInputs_.T.x.raw>joyFaultBand && joyInputs_.T.x.raw<a2dMax-joyFaultBand) ? joyIncGood : joyDecOOR ) ));
    joyInputs_.T.y.health = max(0, min(1000, joyInputs_.T.y.health + ( (joyInputs_.T.y.raw>joyFaultBand && joyInputs_.T.x.raw<a2dMax-joyFaultBand) ? joyIncGood : joyDecOOR ) ));
